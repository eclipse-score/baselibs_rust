...
pub fn set_logger(logger: Box<dyn Log>) -> Result<(), SetLoggerError> {
    LOGGER.set(Box::leak(logger)).map_err(|_| SetLoggerError(()))
}
...
pub fn logger() -> &'static dyn Log {
    static NOP_LOGGER: &'static dyn Log = &NopLogger;
...
mod tests {
    ...
    fn test_set_logger_and_logger() {
        // `set_logger` and `logger` operate on global state.
        ...
        // Set for the second time.
        {
            ...
            let result = set_logger(new_logger);
            assert!(result.is_err());

            // Make sure state didn't change.
            let old_logger = logger();
